...
export class HistoryComponent implements OnInit {
  ...
  getBillNo() {
    this.loader = true;
    this.api.getInvoiceBillNo().subscribe((res: any) => {
      this.loader = false;
  ...
  closeModal(newParty: string): void {
    if (newParty !== '') {
      ...
      this.loader = true;
      this.api.addSellProductPartyName(obj).subscribe((res) => {
        this.loader = false;
        this.productForm.patchValue({
          staticData: {
  ...
  onSubmit(method: any) {
    if (method === 1) {
      if (this.productForm.valid) {
        this.loader = true;
        this.api.addInvoice(this.productForm.value).subscribe((res: any) => {
          this.loader = false;
      ...
      if (this.productForm.valid) {
        ...
        this.api.addInvoice(this.productForm.value).subscribe((res: any) => {
          this.loader = false;
          if (res["status"] === "success") {
            this.router.navigate(['/admin/printBill', res['token']], { queryParams: { method: 'history' } });
          }
          else{
            this.alert("error","Oops...", "Something went wrong..!");
          }
  ...
  getAllPurchaseBill() {
    this.loader = true;
    this.api.getAllInvoiceBill().subscribe((res: any) => {
      this.loader = false;
  ...
  getForEditBill(id: any) {
    this.loader = true;
    this.api.getBill(id, 'history').subscribe((res: any) => {
      if (res["status"] === "success") {
      this.scrollToTop();
      this.loader = false;
      const bill = res["bill"];
  ...
  editPurchesMeterial(method: any) {
    if (method === 1) {
      if (this.productForm.valid) {
        this.loader = true;
        this.api.editInvoice(this.billNo, this.productForm.value).subscribe((res: any) => {
          this.loader = false;
          if (res["status"] === "success") {
