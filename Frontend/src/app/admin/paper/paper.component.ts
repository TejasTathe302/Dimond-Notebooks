...
export class PaperComponent implements OnInit {
  ...
  getBillNo() {
    this.loader = true;
    this.api.getPurchesBillNo().subscribe((res: any) => {
      this.loader = false;
  ...
  closeModal(newParty: string): void {
    if (newParty !== '') {
      ...
      this.loader = true;
      this.api.addPurchesPartyName(obj).subscribe((res) => {
        this.loader = false;
        this.productForm.patchValue({
          staticData: {
  ...
  onSubmit(method: any) {
    if (method === 1) {
      if (this.productForm.valid) {
        this.loader = true;
        this.api.Addbill(this.productForm.value).subscribe((res: any) => {
          this.loader = false;
      ...
      if (this.productForm.valid) {
        ...
        this.api.Addbill(this.productForm.value).subscribe((res: any) => {
          this.loader = false;
          if (res["status"] === "success") {
            this.router.navigate(['/admin/printBill', res['token']], { queryParams: { method: 'paper' } });
          }
          else{
            this.alert("error","Oops...", "Something went wrong..!");
          }
  ...
  getAllPurchaseBill() {
    this.loader = true;
    this.api.getAllPurchesBill().subscribe((res: any) => {
      this.loader = false;
  ...
  getForEditBill(id: any) {
    this.loader = true;
    this.scrollToTop();
    this.api.getBill(id, 'paper').subscribe((res: any) => {
      this.loader = false;
      if (res["status"] === "success") {
        this.scrollToTop();
      const bill = res["bill"];
  ...
  editPurchesMeterial(method: any) {
    if (method === 1) {
      if (this.productForm.valid) {
        this.loader = true;
        this.api.editPurchesMeterial(this.billNo, this.productForm.value).subscribe((res: any) => {
          this.loader = false;
          if (res["status"] === "success") {
