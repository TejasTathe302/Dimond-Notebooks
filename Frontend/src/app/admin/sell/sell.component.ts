...
export class SellComponent implements OnInit {
  ...
  getBillNo() {
    this.loader = true;
    this.api.getSellProductBillNo().subscribe((res: any) => {
      this.loader = false;
  ...
  onSubmit(method: any) {
    if (method === 1) {
      if (this.productForm.valid) {
        this.loader = true;
        this.api.addSellProductMeterial(this.productForm.value).subscribe((res: any) => {
          this.loader = false;
      ...
      if (this.productForm.valid) {
        ...
        this.api.addSellProductMeterial(this.productForm.value).subscribe((res: any) => {
          this.loader = false;
          if (res["status"] === "success") {
            this.router.navigate(['/admin/printBill', res['token']], { queryParams: { method: 'sell' } });
          }
          else{
            this.alert("error","Oops...", "Something went wrong..!");
          }
  ...
  ManBilldet() {
    ...
    if (this.billdet) {
      this.getAllSellProductBill();
    }
  }
  getForEditBill(id: any) {
    this.loader = true;
    this.api.getBill(id, 'sell').subscribe((res: any) => {
      if (res["status"] === "success") {
        this.scrollToTop();
      this.loader = false;
      const bill = res["bill"];
  ...
  editPurchesMeterial(method: any) {
    if (method === 1) {
      if (this.productForm.valid) {
        this.loader = true;
        this.api.editSellMeterial(this.billNo, this.productForm.value).subscribe((res: any) => {
          this.loader = false;
          if (res["status"] === "success") {
